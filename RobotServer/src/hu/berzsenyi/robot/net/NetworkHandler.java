...
public class NetworkHandler {
				...
				try {
					...
					if(pkt != null && pkt.getLength() != 0) {
						...
						synchronized (this.net.packetsReceived) {
							this.net.packetsReceived.add(pkt);
						}
					}
				} catch(Exception e) {
					e.printStackTrace();
	...
	public static class UDPSendThread extends Thread {
		...
		public UDPSendThread(NetworkHandler net, byte[] data) {
		...
		public void run() {
			try {
				this.net.socketUdp.send(new DatagramPacket(this.data, this.data.length, this.net.remoteAddress));
			} catch(Exception e) {
				e.printStackTrace();
				if(this.net.open)
					this.net.close();
					...
					try {
						while(open)
							if(socketTcp == null || !socketTcp.isConnected()) {
								...
								if(remoteAddress == null && socketTcp != null)
	...
	public void send(byte[] data, boolean tcp) {
		try {
			if(tcp)
				...
				new UDPSendThread(this, data.clone()).start();
		} catch(Exception e) {
			e.printStackTrace();
			if(this.open)
				this.close();
